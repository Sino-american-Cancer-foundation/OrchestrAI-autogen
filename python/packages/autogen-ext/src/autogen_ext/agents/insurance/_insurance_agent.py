...
from ._prompts import INSURANCE_AGENT_SYSTEM_PROMPT


class InsuranceAgentConfig(BaseModel):
    """Configuration for InsuranceAgent"""
...
class InsuranceAgent(BaseChatAgent, Component[InsuranceAgentConfig]):
    """
    An agent specialized in handling insurance verification tasks through a Model Context Protocol (MCP) server.
    
    This agent connects to an SSE-based MCP server to perform insurance-related operations such as:
    - Logging into insurance portals
    - Analyzing insurance portal screenshots
    - Making calls to verify eligibility
    - Getting results from verification calls
    - Running background tasks on VMs for EMR/EHR systems
    ...
    component_config_schema = InsuranceAgentConfig
    component_provider_override = "autogen_ext.agents.insurance.InsuranceAgent"

    DEFAULT_DESCRIPTION = "An agent specializing in insurance verification and eligibility checking through integration with insurance portals and phone systems."
    
    DEFAULT_SYSTEM_MESSAGE = INSURANCE_AGENT_SYSTEM_PROMPT
    ...
    def _to_config(self) -> InsuranceAgentConfig:
        """Convert to config for serialization."""
        return InsuranceAgentConfig(
            name=self.name,
    ...
    @classmethod
    def _from_config(cls, config: InsuranceAgentConfig) -> Self:
