...
def add_mcp_tools(server_params_list: List[SseServerParams]):
    ...
    for server_params in server_params_list:
        ...
        for adapter in tool_adapters:
            tools.append(_update_component_metadata(adapter.dump_component(), adapter.name, adapter.description))
    return tools
