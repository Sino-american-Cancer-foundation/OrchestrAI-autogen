...
class McpHostAgentConfig(BaseModel):
    ...
    mcp_servers: List[McpServerParams] | None = None
    server_params: McpServerParams | None = None  # Kept for backward compatibility
...
class McpHostAgent(BaseChatAgent, Component[McpHostAgentConfig]):
    ...
    def __init__(
        ...
        *,
        ...
        # Handle both single server_params and multiple mcp_servers for backward compatibility
        self._mcp_servers = []
        if mcp_servers is not None:
            # Check and create appropriate server_params objects if needed
            self._mcp_servers = [check_and_create_server_params(params) for params in mcp_servers]
        elif server_params is not None:
            # Check and create appropriate server_params object if needed
            processed_server_params = check_and_create_server_params(server_params)
            self._mcp_servers = [processed_server_params]
        self._server_params = server_params  # Keep for backward compatibility
    ...
    async def _lazy_init(self) -> None:
        """Initialize the MCP connections and fetch tools on first use."""
        if self._initialized or not self._mcp_servers:
            return
        try:
            # Fetch all available tools from all MCP servers
            all_tools = []
            for server_params in self._mcp_servers:
                server_tools = await mcp_server_tools(server_params)
                all_tools.extend(server_tools)
                self.logger.info(f"Initialized MCP connection with {len(server_tools)} tools")
            ...
            # Check for duplicate tool names across all servers
            if len(tool_names) != len(set(tool_names)):
                duplicate_names = set([name for name in tool_names if tool_names.count(name) > 1])
                self.logger.warning(f"Duplicate tool names found across MCP servers: {duplicate_names}")

            self.logger.info(f"Initialized all MCP connections with a total of {len(self._tools)} tools")
    ...
    def _to_config(self) -> McpHostAgentConfig:
        ...
        # Ensure mcp_servers are properly serialized
        ...
        return McpHostAgentConfig(
            ...
            server_params=server_params  # Keep for backward compatibility           
        )
    ...
    def _from_config(cls, config: McpHostAgentConfig) -> Self:
        ...
        print("Config mcp_servers ...", flush=True)
        mcp_servers = None
        if config.mcp_servers:
            mcp_servers = [check_and_create_server_params(params) for params in config.mcp_servers]
        
        # Process server_params (kept for backward compatibility)
        server_params = None
        if config.server_params:
            server_params = check_and_create_server_params(config.server_params)
