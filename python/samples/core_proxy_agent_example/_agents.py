...
try:
    from ._types import GroupChatMessage, RequestToSpeak, MessageChunk, PatientData, AgentMode, UIAgentConfig
except ImportError:
    from _types import GroupChatMessage, RequestToSpeak, MessageChunk, PatientData, AgentMode, UIAgentConfig
...
class OrchestratorAgent(RoutedAgent):
    ...
    async def handle_message(self, message: GroupChatMessage, ctx: MessageContext) -> None:
        ...
        selector_prompt = f"""You are orchestrating a conversation between healthcare agents:
{roles}.
{plan_context}
...
Read the above conversation. Then select the next agent from {participants} to respond. Follow the established plan when possible. If you think enough conversation has occurred (for example they have talked for {self._max_rounds} rounds), return 'FINISH'.
"""
        ...
        if completion.content.upper() == "FINISH":
            finish_msg = "The conversation has concluded. Thank you for using our healthcare assistant system!"
    ...
    async def _create_complete_plan(self, cancellation_token: CancellationToken) -> None:
        ...
        planning_prompt = f"""You are an orchestrator for a healthcare assistant system. Create a concise 2-3 step plan to address the user's request.
...
Provide a brief plan with 2-3 clear steps maximum. Be concise and focus only on the essential actions needed.
"""
...
""")
        ...
        if response.startswith("CALL_NEEDED:"):
            ...
            await publish_message_to_ui_and_backend(
                runtime=self,
                source=self.id.type,
                user_message="Agent is inactive.",
                ui_config=self._ui_config,
                group_chat_topic_type=self._group_chat_topic_type,
            )
    ...
    async def _generate_response_for_group_chat(self) -> None:
        ...
        try:
            # Get the last external participant message from history
            external_participant_msg = None
            for msg in reversed(self._chat_history):
                if hasattr(msg, 'source') and msg.source == "External_Participant_TwilioProxyAgent":
                    external_participant_msg = msg.content
                    break
            
            if not external_participant_msg:
                # No external participant message found, don't respond yet
                print(f"[TwilioProxyAgent] No external participant message to respond to, skipping response generation")
                return
        ...
        try:
            ...
            call_result = await self.workbench.call_tool(
                ...
                arguments={
                    ...
                    "information": f"Healthcare agent call initiated for {phone_number}"
                },
                ...
                try:
                    ...
                    if msg_type == "speech_segment":
                        ...
                        await publish_message_to_ui(
                            runtime=self,
                            source=self.id.type,
                            user_message=f"Call ended - Eligibility: {eligibility}",
                            ui_config=self._ui_config,
                        )
    ...
    async def _process_websocket_message(self, ws_message: dict, session_id: str) -> None:
        ...
        try:
            ...
            # Note: Response generation is now handled by orchestrator via RequestToSpeak
            # The TwilioProxyAgent will only respond when explicitly asked by the orchestrator
    ...
    def _build_context_for_external_participant(self, current_input: str) -> str:
        """Build context for generating response to external participant."""
        context = []
        
        # Add group chat history for context
        if self._chat_history:
            context.append("Healthcare conversation context:")
            for msg in self._chat_history[-5:]:  # Last 5 messages
                if hasattr(msg, 'source') and hasattr(msg, 'content'):
                    context.append(f"{msg.source}: {msg.content}")
        
        # Add current input
        context.append(f"External participant said: {current_input}")
        context.append("\nGenerate an appropriate response as the healthcare proxy agent:")
    ...
    def _build_context_for_response_generation(self) -> str:
        ...
        context.append("\nYou are the healthcare proxy agent. Generate ONLY your direct response to the external participant without any prefixes or labels:")
    ...
    async def _generate_and_send_response(self, transcript: str, session_id: str) -> None:
        """Generate streaming response for external participant and send it via WebSocket."""
        try:
            # Build context for response generation
            context = self._build_context_for_external_participant(transcript)
            ...
            async for chunk in stream:
                if isinstance(chunk, str):
                    ...
                    # Check if we have a complete sentence
                    if self._detect_sentence_end(current_sentence):
                        # Send the complete sentence as partial response
                        await self._send_response_to_external_participant(
                            current_sentence.strip(),
                            session_id,
                            is_partial=True
            ...
            # Publish complete response to UI only (not to group chat to avoid triggering orchestrator)
            await publish_message_to_ui(
                runtime=self,
                source=self.id.type,
                user_message=cleaned_response,
            ...
            print(f"[TwilioProxyAgent] Error generating response for session {session_id}: {str(e)}")
            import traceback
            traceback.print_exc()
    ...
    async def disconnect_websocket(self) -> None:
        """Disconnect from all WebSocket sessions and clean up resources."""
        try:
            # Clean up all active sessions
            session_ids = list(self.active_websocket_sessions.keys())
            ...
            self.mode = AgentMode.INACTIVE
            self.external_participant_connected = False
            print(f"[TwilioProxyAgent] All WebSocket sessions disconnected")
            
        except Exception as e:
            print(f"[TwilioProxyAgent] Error disconnecting WebSocket: {str(e)}")
...
provide relevant medical data from your database. Simulate a brief search delay before responding with detailed patient information."""
        ...
        # Simulate medical data lookup delay
...
Insurance: {patient_data.insurance_info.get('provider', 'N/A')}
Recent Visits: {len(patient_data.recent_visits)} visits in last 6 months
Emergency Contact: {patient_data.emergency_contact.get('name')}
...
"""
    ...
    async def _get_patient_data(self) -> PatientData:
        ...
        return PatientData(
            ...
            recent_visits=[
                {"date": "2024-01-15", "reason": "Routine checkup", "doctor": "Dr. Wilson"},
                {"date": "2024-02-20", "reason": "Blood pressure monitoring", "doctor": "Dr. Wilson"},
                {"date": "2024-03-10", "reason": "Diabetes follow-up", "doctor": "Dr. Chen"}
            ],
            emergency_contact={
